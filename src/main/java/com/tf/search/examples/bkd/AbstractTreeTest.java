...
import com.tf.search.utils.bkd.BoundingBoxUtils;
...
public abstract class AbstractTreeTest {
    ...
    public void testBasicContains() {
        ...
        Tree tree = getTree(documents.toArray(new Document[documents.size()]), KDBTree.DEFAULT_DOCUMENTS_PER_LEAF);
    ...
    public void testRandomContains() {
        ...
        if (random.nextBoolean()) {
            tree = getTree(documents.toArray(new Document[documents.size()]), random.nextInt(2048) + 2);
        }
        else {
            tree = getTree(documents.toArray(new Document[documents.size()]), KDBTree.DEFAULT_DOCUMENTS_PER_LEAF);
        }
    ...
    public abstract Tree getTree(Document[] documents, int maxDocsPerLeaf);
